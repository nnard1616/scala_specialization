...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  override def equals(obj: Any): Boolean =
    if (obj.isInstanceOf[Tweet]) this.toString.equals(obj.toString)
    else false
}
...
object GoogleVsApple {
  ...
  def filterByContent(keywords: List[String])(n: Tweet): Boolean =
    keywords.exists((t: String) => n.text.contains(t))

  println("A")
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(filterByContent(google))
  println("B")
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(filterByContent(apple))
  println("C")
